                ...
                <form action="{{route('admin.projects.update',$project->slug)}}" method="POST" enctype="multipart/form-data" class="p-4">
                    ...
                    @method('PUT')
                      ...
                      <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('link_git') is-invalid @enderror" id="link_git" name="link_git" value="{{old('link_git', $project->link_git)}}">
                        @error('link_git')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                      </div>
                      <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('lvl_diff') is-invalid @enderror" id="lvl_diff" name="lvl_diff" value="{{old('lvl_diff', $project->lvl_diff)}}">
                        @error('lvl_diff')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                      </div>
